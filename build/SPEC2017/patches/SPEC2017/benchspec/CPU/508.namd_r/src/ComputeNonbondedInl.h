...
inline int pairlist_from_pairlist(BigReal cutoff2,
  ...
  if ( list_size > 16) {
    ...
    //fprintf(stderr, "ED: list_size - 8 %d\n", list_size - 8); 
    int gLastPrivate = 0;
    // TODO: enable pragma only if list_size > 200
    #pragma omp parallel for ordered schedule(static) shared(p_j, list, nli, r2i, jout) private(j0,j1,j2,j3, t_0,t_1,t_2,t_3, r2_0,r2_1,r2_2,r2_3, pj_x_0,pj_x_1,pj_x_2,pj_x_3,pj_y_0,pj_y_1,pj_y_2,pj_y_3,pj_z_0,pj_z_1,pj_z_2,pj_z_3) firstprivate(list_size, p_i_x,p_i_y,p_i_z, cutoff2_delta, r2_delta) lastprivate(gLastPrivate) // ED
    {
    for ( int gg = 0 ; gg < list_size - 4 - 4; gg += 4 ) {
      ...
      pj_x_0 = p_j[j0].position.x; // ED
      pj_x_1 = p_j[j1].position.x; // ED
      pj_x_2 = p_j[j2].position.x; // ED
      pj_x_3 = p_j[j3].position.x; // ED
      pj_y_0 = p_j[j0].position.y; // ED
      pj_y_1 = p_j[j1].position.y; // ED
      pj_y_2 = p_j[j2].position.y; // ED
      pj_y_3 = p_j[j3].position.y; // ED
      pj_z_0 = p_j[j0].position.z; // ED
      pj_z_1 = p_j[j1].position.z; // ED
      pj_z_2 = p_j[j2].position.z; // ED
      pj_z_3 = p_j[j3].position.z; // ED
      ...
      // ED: we need an ordered critical section because it's not guaranteed that index jout will increase at every loop iteration
      //#pragma omp ordered // ED
      {
      nli[ jout ]  = j0;         r2i[ jout ] = r2_0;
      jout += test0;
      nli[ jout ]  = j1;         r2i[ jout ] = r2_1;
      jout += test1;
      nli[ jout ]  = j2;         r2i[ jout ] = r2_2;
      jout += test2;
      nli[ jout ]  = j3;         r2i[ jout ] = r2_3;
      jout += test3;
      }
    ...
    //g -= 4;
    g = gLastPrivate; // ED
    //fprintf(stderr, "ED: g %d\n", g); 
  }

  //fprintf(stderr, "ED: nl1 %p\n", nli); 
  //fprintf(stderr, "ED: r2i %p\n", r2i); 
  //fprintf(stderr, "ED: jout %d\n", jout); 
  //fprintf(stderr, "ED: nli[0] %d\n", nli[0]); 
  //fprintf(stderr, "ED: r2i[0] %f\n", r2i[0]); 
  ...
  //fprintf(stderr, "ED: return value %d\n", nli - newlist); 
  return nli - newlist;
}
