...
#pragma omp parallel \
  private (i, xi, yi, zi, wi, ri, ri1i, sumi, j, k, xij, yij, zij, wij, \
   ...
   stateIDa = caratGetStateWrapper((char*)"egb", 1702); // ED
   {
...
// START FOR
      ...
      for (i = 0; i < prm->Natom; i++) {
         ...
         stateID = caratGetStateWrapper((char*)"egb", 2748); // ED
...
#pragma omp for schedule(dynamic, blocksize)
     ...
     uint64_t stateID2 = 0; // ED

      for (j = 0; j < prm->Natom; j++) {

         stateID2 = caratGetStateWrapper((char*)"egb", 2983); // ED
