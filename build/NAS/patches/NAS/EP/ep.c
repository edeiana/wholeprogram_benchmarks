...
#pragma omp parallel for default(shared) private(i) /// ED VERIFIED
  for (i = 0; i < 2*NK; i++){
    x[i] = -1.0e99;
  }
...
#pragma omp parallel for copyin(x) reduction(+:sx,sy,v0,v1,v2,v3,v4,v5,v6,v7,v8,v9) schedule(static)  
    for (k = 1; k <= np; k++) {
