...
#pragma omp for
    for (j = 1; j <= naa+1; j++) {
stateID = caratGetStateWrapper((char*)"conj_grad", 381); // ED
	q[j] = 0.0;
	z[j] = 0.0;
	r[j] = x[j];
	p[j] = r[j];
...
#pragma omp for reduction(+:rho)
    for (j = 1; j <= lastcol-firstcol+1; j++) {
stateID1 = caratGetStateWrapper((char*)"conj_grad", 398); // ED
	rho = rho + r[j]*r[j];
...
c-------------------------------------------------------------------*/
    for (cgit = 1; cgit <= cgitmax; cgit++) {
      ...
      d = 0.0;
      rho = 0.0;
#pragma omp parallel default(shared) private(j,k,sum,alpha,beta) shared(d,rho0,rho) // ED LOOP
{
...
uint64_t stateID2;
/* rolled version */    
#pragma omp for // ED LOOP
	for (j = 1; j <= lastrow-firstrow+1; j++) {
      stateID2 = caratGetStateWrapper((char*)"conj_grad", 433); // ED
            sum = 0.0;
	    for (k = rowstr[j]; k < rowstr[j+1]; k++) {
    ...
    endStateInvocationWrapper(stateID2);
...
#pragma omp for reduction(+:d)
	for (j = 1; j <= lastcol-firstcol+1; j++) {
stateID3 = caratGetStateWrapper((char*)"conj_grad", 505); // ED
            d = d + p[j]*q[j];
...
#pragma omp for reduction(+:rho)	
	for (j = 1; j <= lastcol-firstcol+1; j++) {
stateID4 = caratGetStateWrapper((char*)"conj_grad", 529); // ED
            z[j] = z[j] + alpha*p[j];
            r[j] = r[j] - alpha*q[j];
//	}
...
#pragma omp for nowait
	for (j = 1; j <= lastcol-firstcol+1; j++) {
stateID5 = caratGetStateWrapper((char*)"conj_grad", 559); // ED
            p[j] = r[j] + beta*p[j];
...
#pragma omp parallel default(shared) private(j,d) shared(sum) // ED LOOP
{
uint64_t stateID6; // ED
#pragma omp for //private(d, k) // ED LOOP
    for (j = 1; j <= lastrow-firstrow+1; j++) {
stateID6 = caratGetStateWrapper((char*)"conj_grad", 580); // ED
	d = 0.0;
	for (k = rowstr[j]; k <= rowstr[j+1]-1; k++) {
            d = d + a[k]*z[colidx[k]];
	}
	r[j] = d;
caratReportStateWrapper(stateID6); // ED
    }
endStateInvocationWrapper(stateID6); // ED

/*--------------------------------------------------------------------
c  At this point, r contains A.z
c-------------------------------------------------------------------*/
uint64_t stateID7; // ED
#pragma omp for reduction(+:sum)
    for (j = 1; j <= lastcol-firstcol+1; j++) {
stateID7 = caratGetStateWrapper((char*)"conj_grad", 596); // ED
	d = x[j] - r[j];
	sum = sum + d*d;
caratReportStateWrapper(stateID7); // ED
    }
endStateInvocationWrapper(stateID7); // ED
} //end omp parallel
    (*rnorm) = sqrt(sum);
}
