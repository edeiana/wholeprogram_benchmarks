...
c-------------------------------------------------------------------*/
{
#pragma omp parallel for default(shared) private(j) // ED VERIFIED
    for (j = 1; j <= naa+1; j++) {
	q[j] = 0.0;
	z[j] = 0.0;
	r[j] = x[j];
...
c-------------------------------------------------------------------*/
#pragma omp parallel for default(shared) private(j) reduction(+:rho) // ED VERIFIED
    for (j = 1; j <= lastcol-firstcol+1; j++) {
	rho = rho + r[j]*r[j];
    }
}/* end omp parallel */
...
/* rolled version */    
#pragma omp parallel for default(shared) private(j,k,sum) // ED VERIFIED
	for (j = 1; j <= lastrow-firstrow+1; j++) {
            sum = 0.0;
	    for (k = rowstr[j]; k < rowstr[j+1]; k++) {
...
c  Obtain p.q
c-------------------------------------------------------------------*/
#pragma omp parallel for default(shared) private(j) reduction(+:d) // ED VERIFIED
	for (j = 1; j <= lastcol-firstcol+1; j++) {
            d = d + p[j]*q[j];
	}
//#pragma omp barrier
...
{
#pragma omp parallel for default(shared) private(j,k,d) //private(d, k) // ED VERIFIED
    for (j = 1; j <= lastrow-firstrow+1; j++) {
	d = 0.0;
...
c-------------------------------------------------------------------*/
#pragma omp parallel for default(shared) private(j,d) reduction(+:sum) // ED VERIFIED
    for (j = 1; j <= lastcol-firstcol+1; j++) {
	d = x[j] - r[j];
	sum = sum + d*d;
    }
