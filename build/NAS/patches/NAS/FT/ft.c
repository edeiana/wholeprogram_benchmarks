...
#pragma omp parallel for default(shared) private(i,j,k) // ED VERIFIED
  for (k = 0; k < d[2]; k++) {
    for (j = 0; j < d[1]; j++) {
      for (i = 0; i < d[0]; i++) {
...
#pragma omp parallel for default(shared) private(i,j,k,ii,ii2,jj,ij2,kk) // ED VERIFIED
...
#pragma omp parallel for default(shared) private(i,j,k,jj) shared(is) // ED VERIFIED
    for (k = 0; k < d[2]; k++) {
      dcomplex y0[NX][FFTBLOCKPAD];
      dcomplex y1[NX][FFTBLOCKPAD];
...
#pragma omp parallel for default(shared) private(i,j,k,ii) shared(is) // ED VERIFIED
    for (k = 0; k < d[2]; k++) {
      dcomplex y0[NX][FFTBLOCKPAD];
      dcomplex y1[NX][FFTBLOCKPAD];
...
//#pragma omp parallel default(shared) 
  {
    //uint64_t stateID = caratGetStateWrapper((char*)"checksum", 853); // ED
    /*--------------------------------------------------------------------
      c-------------------------------------------------------------------*/
    ...
    double chkReal = 0.0; // ED
    double chkImag = 0.0; // ED

#pragma omp parallel for default(shared) private(j,q,r,s) reduction(+:chkReal,chkImag) // ED VERIFIEDED
    for (j = 1; j <= 1024; j++) {
      q = j%NX+1;
      if (q >= xstart[0] && q <= xend[0]) {
        ...
        if (r >= ystart[0] && r <= yend[0]) {
          ...
          if (s >= zstart[0] && s <= zend[0]) {
            chkReal += u1[s-zstart[0]][r-ystart[0]][q-xstart[0]].real; // ED
            chkImag += u1[s-zstart[0]][r-ystart[0]][q-xstart[0]].imag; // ED
