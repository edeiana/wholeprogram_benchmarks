...
static void psinv( double ***r, double ***u, int n1, int n2, int n3,
  ...
  int i3, i2, i1;
  double r1[M], r2[M];
#pragma omp parallel for default(shared) private(i1,i2,i3,r1,r2) // ED VERIFIED 
  for (i3 = 1; i3 < n3-1; i3++) {
    for (i2 = 1; i2 < n2-1; i2++) {
      for (i1 = 0; i1 < n1; i1++) {
...
static void resid( double ***u, double ***v, double ***r,
  ...
  int i3, i2, i1;
  double u1[M], u2[M];
#pragma omp parallel for default(shared) private(i1,i2,i3,u1,u2) // ED VERIFIED
  for (i3 = 1; i3 < n3-1; i3++) {
    for (i2 = 1; i2 < n2-1; i2++) {
      for (i1 = 0; i1 < n1; i1++) {
...
#pragma omp parallel for default(shared) private(j1,j2,j3,i1,i2,i3,x1,y1,x2,y2) // ED VERIFIED
...
static void interp( double ***z, int mm1, int mm2, int mm3,
  ...
  if ( n1 != 3 && n2 != 3 && n3 != 3 ) {
#pragma omp parallel for default(shared) private(i1,i2,i3,z1,z2,z3) // ED VERIFIED
...
#pragma omp parallel for default(shared) private(i1,i2,i3,a) reduction(+:s) reduction(max:tmp) // ED VERIFIED
  for (i3 = 1; i3 < n3-1; i3++) {
    for (i2 = 1; i2 < n2-1; i2++) {
      for (i1 = 1; i1 < n1-1; i1++) {
...
#pragma omp parallel private(i3, i2, x1, x0, d1, xx, a1, rdummy) shared(e3, e2, z)
#pragma omp single
  { 
    for (i3 = 1; i3 < e3; i3++) {
#pragma omp task private(x1, i2, xx, rdummy) firstprivate(x0, d1) shared(z)
      {
        x1 = x0;
        for (i2 = 1; i2 < e2; i2++) {
          xx = x1;
...
#pragma omp parallel for private(i2, i1) // ED VERIFIED
  for (i3 = 0; i3 < n3; i3++) {
    for (i2 = 0; i2 < n2; i2++) {
      for (i1 = 0; i1 < n1; i1++) {
        z[i3][i2][i1] = 0.0;
...
static void zero3(double ***z, int n1, int n2, int n3) {
  ...
  int i1, i2, i3;
#pragma omp parallel for private(i1,i2,i3) // ED VERIFIED
  for (i3 = 0;i3 < n3; i3++) {
    for (i2 = 0; i2 < n2; i2++) {
      for (i1 = 0; i1 < n1; i1++) {
        z[i3][i2][i1] = 0.0;
